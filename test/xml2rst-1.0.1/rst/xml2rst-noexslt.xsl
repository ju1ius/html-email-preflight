...
xml2rst-noexslt.xsl - An XSLT script to convert Docutils XML to reStructuredText
...
Today B<xml2rst-noexslt.xsl> is not the recommended way to use
B<xml2rst>. The preferred way is to install B<lxml> and use
B<xml2rst.py> instead.

=head1 DESCRIPTION

B<xml2rst-noexslt.xsl> is an XSLT script to convert Docutils XML to
...
However, the coverage of Docutils features of B<xml2rst-noexslt.xsl> is
